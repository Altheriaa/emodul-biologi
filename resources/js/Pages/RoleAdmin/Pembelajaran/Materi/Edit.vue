...
const form = useForm({
    ...
    pertanyaan_essay: props.materi?.essay_questions?.length ? props.materi.essay_questions.map(q => ({ id: q.id, pertanyaan: q.pertanyaan })) : [{ pertanyaan: '' }],
    cover_path: null,
...
const submit = () => {
    form.post(`/admin/pembelajaran/materi/${props.materi.id}`, {
        forceFormData: true,
        preserveScroll: true,
    });
};
