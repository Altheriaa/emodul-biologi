...
const cancel = () => router.visit('/admin/evaluasi/bank-soal', { preserveScroll: true });

const submitQuiz = () => {
    quizForm.put(`/admin/evaluasi/bank-soal/${props.quiz.id}`, { preserveScroll: true });
};
...
const submitQuestion = () => {
    ...
    if (editingQuestion.value) {
        questionForm.transform((data) => ({
            ...
            `/admin/evaluasi/bank-soal/${props.quiz.id}/soal/${editingQuestion.value.id}`,
            { onSuccess: closeModal, preserveScroll: true, forceFormData: true }
        );
    } else {
        questionForm.post(
            `/admin/evaluasi/bank-soal/${props.quiz.id}/soal`,
            { onSuccess: closeModal, preserveScroll: true, forceFormData: true }
        );
    }
};
...
const confirmDelete = (q) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(`/admin/evaluasi/bank-soal/${props.quiz.id}/soal/${q.id}`, {
                preserveScroll: true,
            });
        }
    });
