    ...
    <Layout>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4">
            ...
            <div class="bg-white border border-gray-200 rounded-xl p-4 sm:p-5">
                <h2 class="font-bold text-base text-gray-800 mb-4">Hasil Quiz Terbaru</h2>
                <div class="space-y-3 sm:space-y-4">
                    <div v-for="res in recent_results" :key="res.id" class="flex items-center gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-gray-800 truncate">{{ res.user_name }}</p>
                            <p class="text-xs text-gray-400 truncate">{{ res.quiz_title }}</p>
                        </div>
                        <span class="text-sm font-bold" :class="res.is_passed ? 'text-green-600' : 'text-red-500'">{{ res.score }}</span>
                    </div>
